...
function departmentHighestAverageSalary(commands) {
    const departments = new Map();
    const numOfEmployees = Number(commands[0]);
    const employees = commands.slice(1);
    let i = 0;
    while (i < numOfEmployees) {
        let [name, salaryAsStr, position, departmentName, email, ageAsStr] = employees[i].split(" ");
...
//# sourceMappingURL=company-roster.js.map