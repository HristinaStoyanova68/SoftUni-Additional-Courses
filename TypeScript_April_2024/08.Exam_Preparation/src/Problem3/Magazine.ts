...
export class Magazine {
    ...
    removeCloth(color: string): boolean {
      const indexOfCloth = this.clothes.findIndex(cloth => cloth.color === color);
    ...
    getSmallestCloth(): Cloth | null {
      ...
      return this.clothes.reduce((smallest, cloth) => cloth.size < smallest.size ? cloth : smallest);
    }
    ...
    report(): string {
      ...
      const formatedCloth = sortedClothes.map(cloth => cloth.toString()).join('\n');
